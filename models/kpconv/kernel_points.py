...
# Colors for printing
class bcolors:  # {:} -> bcolors.WARNING
    HEADER = '\033[95m'  # purple
    OKBLUE = '\033[94m'  # blue
    OKGREEN = '\033[92m'  # green
    WARNING = '\033[93m'  # yellow
    FAIL = '\033[91m'  # red
    ENDC = '\033[0m'  # reset the color
...
def create_3D_rotations(axis, angle):
    ...
    Code from wikipedia on quaternions and spatial rotation
    :param axis: float32[N, 3]
    :param angle: float32[N,]
    :return: float32[N, 3, 3]
    Example:
    rotations axis:[(x1,y1,z1),(x2,y2,z2),...]  ->  unit vector:x^2+y^2+z^2=1
    rotations axis:[angle1,angle2,...]
    return rotation matrix about xyz& angle
    """
...
def spherical_Lloyd(radius, num_cells, dimension=3, long_radius=3, fixed='center', approximation='monte-carlo',
    ...
    :param long_radius: the long axis of ellipsoid
    ...
    # generate points on the surface of balls between radius r and 0.9r
    ...
    while kernel_points.shape[0] < num_cells:
        ...
        d2 = np.sum((kernel_points[:, 0] ** 2 * long_radius, kernel_points[:, 1] ** 2 * long_radius,
                     kernel_points[:, 2] ** 2), axis=0)
    ...
    # Optional fixing
    # 'center': The first point is [0,0,0]
    # 'verticals'The first three point is [0,0,0],[0,0,0.667],[0,0,-0.667]
    if fixed == 'center':
        kernel_points[0, :] *= 0
    if fixed == 'verticals':
        kernel_points[:3, :] *= 0
    ...
    if approximation == 'discretization':
        # generate 'evenly distributed' points within the ball
        side_n = int(np.floor(approx_n ** (1. / dimension)))  # dim:4,approx_x:5000,side_n:8
        dl = 2 * radius0 / side_n  # dl:0.25
        coords = np.arange(-radius0 + dl / 2, radius0,
                           dl)  # xyzt:[-0.875 -0.625 -0.375 -0.125  0.125  0.375  0.625  0.875]
        if dimension == 2:
    ...
    d2 = np.sum((X[:, 0] ** 2 * long_radius, X[:, 1] ** 2 * long_radius, X[:, 2] ** 2), axis=0)
    ...
    for iter in range(max_iter):
        ...
        # find the nearest kernel points(index:100)
        ...
        for c in range(num_cells):
            ...
            if num_c > 0:
                #  get all neighboring points for one kernel point
                # calculate the centroid and add 'centers'
        ...
        moves = (1 - momentum) * (centers - kernel_points)  # 10% moving distance
        kernel_points += moves
...
def kernel_point_optimization_debug(radius, num_points, num_kernels=1, dimension=3, long_radius=3,
    ...
    # generate num_kernels * num_points points within the balls with radius 0.5r
    ...
    for iter in range(10000):
        ...
        X = A - B
        interd2 = np.sum((X[:, :, :, 0] ** 2 * long_radius, X[:, :, :, 1] ** 2 * long_radius,
                          X[:, :, :, 2] ** 2), axis=0)
        ...
        gradients_norms = np.sqrt(np.sum((gradients[:, :, 0] ** 2 * long_radius,
                                          gradients[:, :, 1] ** 2 * long_radius, gradients[:, :, 2] ** 2), axis=0))
    ...
    r = np.sqrt(np.sum((kernel_points[:, :, 0] ** 2 * long_radius,
                        kernel_points[:, :, 1] ** 2 * long_radius, kernel_points[:, :, 2] ** 2), axis=0))
...
def load_kernels(radius, num_kpoints, dimension, fixed, lloyd=False, long_radius=1, direction=None):
    # Kernel directory
    kernel_dir = 'models/kpconv/kernels'
    ...
    R = np.eye(dimension)  # unit vector
    ...
    if not direction:
        ...
        if dimension == 2:
            ...
            if fixed != 'vertical':
                ...
                # Create the rotation matrix with this vector and angle 3*3
        ...
        initial_vector = np.array([0.0, 0.0, 1.0])
        direction = np.array(direction)
        target_vector = direction / np.linalg.norm(direction)
        ...
        # Rotate kernels
        kernel_points = rotation.apply(kernel_points)

    # kernel_file = join(kernel_dir, 'k_{:03d}_{:s}_{:d}D2.ply'.format(num_kpoints, fixed, dimension))
    # write_ply(kernel_file, kernel_points, ['x', 'y', 'z'])
...
if __name__ == '__main__':
    '''generate some kernel points within the range of the ball'''
    data = load_kernels(1, 15, 3, 'center')
    data = load_kernels(1, 16, 3, 'center', long_radius=3, direction=(1, 1, 1))
